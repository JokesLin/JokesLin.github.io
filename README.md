...
```
index.html                  门户首页 — 密码锁屏、星空动效、游戏入口横幅、七模块导航
...
- 密码锁屏验证，星空粒子动效背景
- 暗黑赛博朋克风游戏入口横幅（Hero区下方），脉冲动画 + 粒子特效
- 七模块导航卡片，悬浮渐变动效
...
- 单文件网页游戏，暗黑赛博朋克风格，浏览器直接运行
- 肉鸽生存玩法：自动攻击 + 走位策略，12波敌人挑战
- 10位角色（基于实验室同学设定），每位有独特被动技能和专属故事事件
- 13种武器系统，支持同种合并升级（最高+7/MAX）
- 波间商店：武器区 / 增益区 / 遗物区，金币经济 + 3种难度
- 多步骤随机校园故事事件（组会风云、深夜食堂、506宿舍之夜等）
- 场景区域Buff系统（宿舍区回血、食堂加血、操场加速等9个区域）
- 连杀系统、掉落物系统、小地图、角色选择界面
...
- 游戏入口横幅：暗黑赛博朋克风，霓虹渐变 + 脉冲动画（`assets/game-banner.css`）
